...
export const PRIVACY_SECTIONS = {
  ...
  en: [
    ...
    },
    {
      ...
      body:
        'If you use the photo analysis, the label photos you take are downscaled and transmitted once for evaluation: first to a function we operate at Supabase, from there to the Anthropic API (Claude), which reads the label text. The photos are stored neither at Supabase nor in any app database; only the structured result is returned. If a barcode was scanned before the photo analysis, the extracted label result (product name, ingredients, dosage) is stored together with the barcode number in a shared product database at Supabase, so that the same product can be recognised in the future without a new evaluation. This entry describes only the product; it contains no photos and no data about you. Processing may take place on servers outside the EU (in particular the USA). This transmission only happens after you have explicitly agreed to it in the app. You can withdraw this consent at any time in the settings; photo scanning will then no longer be available. Legal basis: consent (Art. 6(1)(a) GDPR).',
    },
    {
