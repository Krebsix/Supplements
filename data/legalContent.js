 ...
 * supabaseClient.js, PurchaseLogic.js, PurchaseStore.js) und gegen die
 * Website (web/: Vercel-Hosting, Beta-Formular an Loops). Wer einen
...
export const PRIVACY_SECTIONS = {
  de: [
    ...
    {
      heading: 'Website (mysuplea.com)',
      body:
        'Diese Erklärung gilt auch für unsere Website. Hosting: Die Seiten liefert unser Auftragsverarbeiter Vercel aus (Vercel Inc., USA, Auftragsverarbeitungsvertrag mit Standardvertragsklauseln). Beim Abruf entstehen technisch Server-Protokolle mit IP-Adresse, Zeitpunkt, aufgerufener Seite und Browser-Kennung; sie dienen dem sicheren Betrieb und werden nach kurzer Zeit gelöscht. Die Website setzt keine Cookies, lädt keine Schriften oder Skripte von Dritten und enthält keine Analyse- oder Werbedienste. Rechtsgrundlage: berechtigtes Interesse an einem sicheren Betrieb (Art. 6 Abs. 1 lit. f DSGVO). Beta-Anmeldung: Trägst du dich für die Beta ein, speichert unser Auftragsverarbeiter Loops (Astrodon Corporation, USA, Auftragsverarbeitungsvertrag, Standardvertragsklauseln und EU-US Data Privacy Framework) deine E-Mail-Adresse, den Zeitpunkt, die gewählte Sprache und den Nachweis deiner Bestätigung. Eingetragen bist du erst, wenn du den Link in der Bestätigungsmail anklickst. Zweck ist ausschließlich die Einladung zur Beta und Hinweise zu ihrem Verlauf. Rechtsgrundlage: Einwilligung (Art. 6 Abs. 1 lit. a DSGVO), jederzeit widerrufbar über den Abmeldelink in jeder Mail oder per Nachricht an uns; danach wird die Adresse gelöscht. Spätestens nach Ende der Beta löschen wir die Liste.',
    },
    {
  ...
  en: [
    ...
    },
    {
      heading: 'Website (mysuplea.com)',
      body:
        'This statement also covers our website. Hosting: the pages are delivered by our processor Vercel (Vercel Inc., USA, data processing agreement with standard contractual clauses). Each visit technically creates server logs with IP address, time, requested page and browser identifier; they serve secure operation and are deleted after a short time. The website sets no cookies, loads no third-party fonts or scripts and contains no analytics or advertising services. Legal basis: legitimate interest in secure operation (Art. 6(1)(f) GDPR). Beta sign-up: if you sign up for the beta, our processor Loops (Astrodon Corporation, USA, data processing agreement, standard contractual clauses and EU-US Data Privacy Framework) stores your email address, the time, the chosen language and proof of your confirmation. You are only signed up once you click the link in the confirmation email. The sole purpose is the invitation to the beta and notes on its progress. Legal basis: consent (Art. 6(1)(a) GDPR), revocable at any time via the unsubscribe link in every email or by contacting us; the address is then deleted. We delete the list at the latest when the beta ends.',
    },
    {
      heading: 'Your rights',
      body:
