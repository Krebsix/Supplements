...
- Neues Projekt anlegen, zwei Apps darin verbinden: die iOS-App (App-Store-
  Connect-API-Key, in App Store Connect unter Users and Access → Keys
  erzeugen) und die Android-App (Play-Service-Account-JSON, in der Google
  Cloud Console fuer das Play-Console-Projekt erzeugen).
...
- Zahlungsproblem bei Apple simulieren (Sandbox-Testkonto mit hinterlegtem
  "Ask to Buy" oder ueber die Apple-eigenen Testkarten fuer fehlgeschlagene
  Zahlungen) und pruefen, dass die App den Fehlerfall abfaengt statt
  abzustuerzen.
