...
| `SearchPlan.js` | Suche im Tab "Wissen": Wirkstoff-Freitextsuche (`searchSubstances`) und die Rolle der Produkte nach Namen (`planProductHits` ueber `searchSeedCatalog`): bei erkannter Beschwerde oder genau einem erkannten Wirkstoff keine Namenstreffer, bei mehreren Wirkstoffen nachrangig, ohne Wirkstoff als Antwort. Tipp auf ein Produkt fuehrt nach `CATALOG_PICK_ROUTE` |
...
| `CloudBackup.js` | Verschluesselter Stand je Konto: Rundtrip zwischen Haupt-Store-Zustand und Ciphertext (AES-256-GCM ueber `AccountCrypto.js`), Login-Entscheidung (`decideOnLogin`: none/upload/restore/ask) und Zeitformat fuer die Oberflaeche. `hasLocalData` entscheidet, ob lokal etwas liegt, das verloren gehen koennte: Es vergleicht die Nutzerdaten-Felder (`LOCAL_DATA_FIELDS`) mit dem Ausgangszustand, NICHT alle `BACKUP_DATA_FIELDS` — Einrichtungszustand (Onboarding, Einwilligungen, Sprache, Lebensphase, Kontingente) ist auf jedem eingerichteten Geraet gesetzt und wuerde den `restore`-Zweig unerreichbar machen. Rein, ohne Store und Netz |
| `CloudBackupStore.js` / `useCloudBackupStore.js` | zustand-Factory fuer das Cloud-Backup, getrennt vom Haupt-Store: buendelt Aenderungen zu einem verzoegerten, gebuendelten Upload (`createCoalescedRunner`), fuehrt den Login-Check aus und haelt den Dialog fuer widerspruechliche Staende (`pendingDecision`). Ein nicht lesbarer Server-Stand wird NIE still ersetzt, aus keinem Grund (falscher Schluessel, neuere App-Version, ungueltiges JSON, fremdes Schema, fehlende Daten): Es gibt eine Rueckfrage plus die persistierte Schreibsperre `uploadBlocked`, die den Neustart ueberlebt und auch manuelles Sichern blockiert. `useCloudBackupStore.js` bindet die echten Abhaengigkeiten |
